#
# cleanup-env.ps1 - NemesisBot Rust 项目测试环境清理脚本 (PowerShell)
#
# 功能：
...
# 尝试通过 PID 文件停止
if (Test-Path "test-tools\autotest\nemesisbot.pid") {
    $nemesisPid = Get-Content "test-tools\autotest\nemesisbot.pid"
...
# 尝试通过 PID 文件停止
if (Test-Path "test-tools\autotest\testaiserver.pid") {
    $testaiPid = Get-Content "test-tools\autotest\testaiserver.pid"
    ...
    Remove-Item -Path "test-tools\autotest\testaiserver.pid" -Force -ErrorAction SilentlyContinue
}
...
Write-Host "Note: test-tools\autotest\ directory was not removed (AI should handle this)"
