...
def test_read_ws_endpoint_from_config(tmp_path):
    ...
    post-bind port wins when present."""
    ws = tmp_path
    ...
    port, token = _read_ws_endpoint(ws, DEFAULT_WEB_PORT)
    assert port == 51234
    assert token == "tok123"
    # post-bind state overrides the configured port
...
def test_read_ws_endpoint_defaults_when_missing(tmp_path):
    port, token = _read_ws_endpoint(tmp_path, DEFAULT_WEB_PORT)
