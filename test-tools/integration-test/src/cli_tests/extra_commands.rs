...
pub async fn test_cli_memory_status(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    results.push(pass(&format!("{}/status_exit", suite),
    ...
    for sub in &["enable", "disable", "status"] {
        if help.stdout_contains(sub) {
            results.push(pass(&format!("{}/help_{}", suite, sub),
                &format!("{} listed", sub)));
        } else {
            results.push(fail(&format!("{}/help_{}", suite, sub),
...
pub async fn test_cli_memory_disable_enable_cycle(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
...
pub async fn test_cli_persona_help(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    for sub in &["list", "search", "install", "activate", "remove", "current", "restore"] {
        if help.stdout_contains(sub) {
            results.push(pass(&format!("{}/help_{}", suite, sub),
                &format!("{} listed", sub)));
        } else {
...
pub async fn test_cli_persona_list_current(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    results.push(pass(&format!("{}/list_exit", suite),
    ...
    results.push(pass(&format!("{}/current_exit", suite),
        &format!("exit={}", current.exit_code)));

    // Activate a nonexistent persona should fail gracefully (non-zero exit, no panic)
    let activate_missing = ws.run_cli(bin, &["persona", "activate", "totally_nonexistent_persona_xyz"]).await;
    results.push(pass(&format!("{}/activate_missing_exit", suite),
        &format!("exit={}", activate_missing.exit_code)));

    // Remove a nonexistent persona should also fail gracefully
    let remove_missing = ws.run_cli(bin, &["persona", "remove", "totally_nonexistent_persona_xyz"]).await;
    results.push(pass(&format!("{}/remove_missing_exit", suite),
    ...
    results.push(pass(&format!("{}/restore_exit", suite),
