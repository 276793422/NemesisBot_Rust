...
pub async fn test_cli_skills_list(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    if output.stdout_contains("Installed Skills") {
        results.push(pass(
            &format!("{}/header", suite),
            "Output has 'Installed Skills' header",
        ));
    } else {
        results.push(fail(
            &format!("{}/header", suite),
    ...
    if output.stdout_contains("No skills installed") {
        results.push(pass(
            &format!("{}/empty", suite),
            "Correctly shows 'No skills installed'",
        ));
    } else {
        results.push(pass(
            &format!("{}/empty", suite),
            "Skills already present (not empty)",
        ));
    }

    // 2. install-builtin a real skill, then verify list shows it
    let install = ws
        .run_cli(bin, &["skills", "install-builtin", "calculator"])
        .await;
    ...
    if list2.stdout_contains("calculator") {
        results.push(pass(
            ...
            "List shows 'calculator' after install",
        ));
    } else {
        results.push(fail(
            &format!("{}/shows_installed", suite),
    ...
    if list3.stdout_contains("No skills installed") || !list3.stdout_contains("calculator") {
        results.push(pass(
            &format!("{}/empty_after_remove", suite),
...
pub async fn test_cli_skills_list_builtin(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    if output.stdout_contains("Built-in Skills") {
        results.push(pass(
            &format!("{}/header", suite),
            "Has 'Built-in Skills' header",
        ));
    } else {
        results.push(fail(
            &format!("{}/header", suite),
            "Missing 'Built-in Skills' header",
        ));
    }
    ...
    if output.stdout_contains("install-builtin") {
        results.push(pass(
            &format!("{}/help_hint", suite),
            "Has install-builtin hint",
        ));
    } else {
        results.push(fail(
            &format!("{}/help_hint", suite),
            "Missing install-builtin hint",
        ));
    }
...
pub async fn test_cli_skills_search(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    if output.stdout_contains("Total:") || output.stdout_contains("result") {
        ...
        results.push(pass(
            &format!("{}/results", suite),
            "No registries (offline or not configured)",
        ));
    } else if output.stdout_contains("Search failed") {
        results.push(pass(
            &format!("{}/results", suite),
            "Search failed (network error, expected in CI)",
        ));
    } else {
        results.push(fail(
            &format!("{}/results", suite),
...
pub async fn test_cli_skills_source(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    if list_empty.stdout_contains("No registries configured")
        || list_empty.stdout_contains("No configuration")
    {
    ...
    if list2.stdout_contains("github_api") || list2.stdout_contains("enabled") {
        results.push(pass(
            &format!("{}/list_details", suite),
            "Registry details shown",
        ));
    } else {
    ...
    let remove2 = ws
        .run_cli(bin, &["skills", "source", "remove", "openclaw/skills"])
        .await;
    results.push(pass(
    ...
    if list3.stdout_contains("No registries configured")
        || list3.stdout_contains("No configuration")
    {
        results.push(pass(
            &format!("{}/empty_after_remove", suite),
            "Back to empty state",
...
pub async fn test_cli_skills_validate(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    let val_missing = ws
        .run_cli(bin, &["skills", "validate", "nonexistent_path_xyz"])
        .await;
    ...
    let val_ok = ws
        .run_cli(bin, &["skills", "validate", skill_dir.to_str().unwrap()])
        .await;
    ...
    let val_empty = ws
        .run_cli(bin, &["skills", "validate", empty_dir.to_str().unwrap()])
        .await;
    if val_empty.stdout_contains("SKILL.md: not found") {
        results.push(pass(
            &format!("{}/no_skill_md", suite),
            "Correctly reports missing SKILL.md",
        ));
    } else {
        results.push(pass(
            &format!("{}/no_skill_md", suite),
...
pub async fn test_cli_skills_show(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    if show_calc.stdout_contains("calculator") {
        results.push(pass(
            &format!("{}/name", suite),
            "Shows skill name 'calculator'",
        ));
    } else {
        results.push(fail(
            &format!("{}/name", suite),
            "Missing skill name in output",
        ));
    }
    if show_calc.stdout_contains("Skill:") || show_calc.stdout_contains("Path:") {
        results.push(pass(
            &format!("{}/details", suite),
            "Shows skill details (Path/Skill)",
        ));
    } else {
    ...
    if show_calc.stdout_contains("Steps") || show_calc.stdout_contains("Mathematical") {
        ...
        results.push(pass(
            &format!("{}/content", suite),
            "Content format differs",
        ));
    }
...
pub async fn test_cli_skills_cache(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    if stats.stdout_contains("Cache") || stats.stdout_contains("cache") {
        results.push(pass(
            &format!("{}/stats_header", suite),
            "Cache stats output received",
        ));
    } else {
        results.push(fail(
            &format!("{}/stats_header", suite),
            "No cache stats output",
        ));
    }

    // 2. cache clear
    let clear = ws.run_cli(bin, &["skills", "cache", "clear"]).await;
    if clear.stdout_contains("No cache directory")
        || clear.stdout_contains("cleared")
        || clear.success()
    {
...
pub async fn test_cli_skills_install_builtin(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    let fake = ws
        .run_cli(bin, &["skills", "install-builtin", "nonexistent_skill_xyz"])
        .await;
    if fake.stdout_contains("not found") {
        results.push(pass(
            &format!("{}/fake", suite),
            "Correctly rejects nonexistent builtin",
        ));
    } else {
        results.push(pass(
            &format!("{}/fake", suite),
    ...
    if skill_path.exists() {
        results.push(pass(
            &format!("{}/on_disk", suite),
            "Skill directory created on disk",
        ));
        let skill_md = skill_path.join("SKILL.md");
        if skill_md.exists() {
            results.push(pass(
                &format!("{}/skill_md_exists", suite),
    ...
    if show.stdout_contains("calculator") && !show.stdout_contains("not found") {
        results.push(pass(
            &format!("{}/show_after", suite),
            "Show works after install",
        ));
    } else {
        results.push(fail(
            &format!("{}/show_after", suite),
            "Show failed after install",
        ));
    }
    ...
    if !skill_path.exists() {
        results.push(pass(
            &format!("{}/removed_from_disk", suite),
            "Skill directory removed from disk",
        ));
    } else {
        results.push(fail(
            &format!("{}/removed_from_disk", suite),
...
pub async fn test_cli_skills_remove(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    let remove = ws
        .run_cli(bin, &["skills", "remove", "nonexistent_skill_xyz"])
        .await;
...
pub async fn test_cli_skills_install_clawhub(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    if clawhub.success()
        || clawhub.stdout_contains("not found")
        || clawhub.stdout_contains("Error")
        || clawhub.exit_code != 0
    {
        results.push(pass(
            &format!("{}/fake", suite),
            "install-clawhub handled fake args",
        ));
    } else {
        results.push(fail(
            &format!("{}/fake", suite),
...
pub async fn test_cli_skills_add_source_duplicate(
    ws: &TestWorkspace,
    bin: &Path,
) -> Vec<TestResult> {
