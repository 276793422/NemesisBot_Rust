...
pub async fn test_cli_security_status(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    if output.stdout_contains("Security") || output.stdout_contains("security") || output.success()
    {
        results.push(pass(
            &format!("{}/output", suite),
            "Security status output received",
        ));
    } else {
        results.push(fail(
            &format!("{}/output", suite),
...
pub async fn test_cli_security_audit(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    // audit show --limit 5
    let show_limited = ws
        .run_cli(bin, &["security", "audit", "show", "--limit", "5"])
        .await;
    results.push(pass(
    ...
    let export = ws
        .run_cli(
            ...
            &[
                ...
                "audit",
                "export",
                export_path.to_str().unwrap_or("audit.json"),
            ],
        )
        .await;
    results.push(pass(
...
pub async fn test_cli_security_rules(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    let list_filtered = ws
        .run_cli(bin, &["security", "rules", "list", "file_path"])
        .await;
    results.push(pass(
...
pub async fn test_cli_security_approve_deny(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    let approve = ws
        .run_cli(bin, &["security", "approve", "nonexistent-id"])
        .await;
    results.push(pass(
    ...
    let deny = ws
        .run_cli(
            bin,
            &["security", "deny", "nonexistent-id", "test", "reason"],
        )
        .await;
    results.push(pass(
        &format!("{}/deny", suite),
    ...
    let config_reset_help = ws
        .run_cli(bin, &["security", "config-reset", "--help"])
        .await;
    results.push(pass(
