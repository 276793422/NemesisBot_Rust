...
pub async fn test_cli_scanner_enable_disable(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    let disable = ws
        .run_cli(bin, &["scanner", "disable", "temp-engine"])
        .await;
    results.push(pass(
    ...
    let install_dir = ws
        .run_cli(
            ...
            &[
                ...
                "install",
                "--dir",
                ws.path().join("scanner").to_str().unwrap_or("./scanner"),
            ],
        )
        .await;
    results.push(pass(
...
pub async fn test_cli_scanner_download_test_update(
    ws: &TestWorkspace,
    bin: &Path,
) -> Vec<TestResult> {
