...
pub async fn test_cli_cors_full(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    // cors add
    let add = ws
        .run_cli(bin, &["cors", "add", "http://localhost:3000"])
        .await;
    results.push(pass(
        &format!("{}/add", suite),
    ...
    // cors add --cdn
    let add_cdn = ws
        .run_cli(bin, &["cors", "add", "https://cdn.example.com", "--cdn"])
        .await;
    results.push(pass(
    ...
    // cors validate
    let validate_ok = ws
        .run_cli(bin, &["cors", "validate", "http://localhost:3000"])
        .await;
    results.push(pass(
    ...
    let validate_fail = ws
        .run_cli(bin, &["cors", "validate", "http://evil.com"])
        .await;
    results.push(pass(
    ...
    // cors remove
    let remove = ws
        .run_cli(bin, &["cors", "remove", "http://localhost:3000"])
        .await;
    results.push(pass(
        &format!("{}/remove", suite),
    ...
    // cors remove --cdn
    let remove_cdn = ws
        .run_cli(bin, &["cors", "remove", "https://cdn.example.com", "--cdn"])
        .await;
    results.push(pass(
