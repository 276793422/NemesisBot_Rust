...
pub async fn test_cli_agent_set_llm(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    let output = ws
        .run_cli(bin, &["agent", "set", "llm", "test/testai-1.1"])
        .await;
    results.push(pass(
...
pub async fn test_cli_agent_set_concurrent(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    // Set reject mode
    let reject = ws
        .run_cli(bin, &["agent", "set", "concurrent-mode", "reject"])
        .await;
    results.push(pass(
        &format!("{}/reject", suite),
    ...
    // Set queue mode
    let queue = ws
        .run_cli(bin, &["agent", "set", "concurrent-mode", "queue"])
        .await;
    results.push(pass(
        &format!("{}/queue", suite),
...
pub async fn test_cli_agent_message(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    if help.stdout_contains("--message") || help.stdout_contains("-m") {
        results.push(pass(
            &format!("{}/message_flag", suite),
            "--message flag present",
        ));
    } else {
        results.push(fail(
            &format!("{}/message_flag", suite),
    ...
    if help.stdout_contains("--session") || help.stdout_contains("-s") {
        results.push(pass(
            &format!("{}/session_flag", suite),
            "--session flag present",
        ));
    } else {
        results.push(fail(
            &format!("{}/session_flag", suite),
            "--session flag missing",
        ));
    }
    ...
    if help.stdout_contains("--no-console") {
        results.push(pass(
            &format!("{}/no_console_flag", suite),
            "--no-console flag present",
        ));
    } else {
        results.push(fail(
            &format!("{}/no_console_flag", suite),
            "--no-console flag missing",
        ));
    }
