...
pub async fn test_cli_channel_web(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    // web auth-set
    let auth_set = ws
        .run_cli(bin, &["channel", "web", "auth-set", "test-token-123"])
        .await;
    results.push(pass(
    ...
    // web host
    let host = ws
        .run_cli(bin, &["channel", "web", "host", "127.0.0.1"])
        .await;
    results.push(pass(
        &format!("{}/host", suite),
    ...
    // Restore auth token
    let _ = ws
        .run_cli(bin, &["channel", "web", "auth-set", "276793422"])
        .await;
...
pub async fn test_cli_channel_websocket(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    // websocket set host
    let set = ws
        .run_cli(bin, &["channel", "websocket", "set", "host", "127.0.0.1"])
        .await;
    results.push(pass(
    ...
    // websocket get host
    let get = ws
        .run_cli(bin, &["channel", "websocket", "get", "host"])
        .await;
    results.push(pass(
...
pub async fn test_cli_channel_external(ws: &TestWorkspace, bin: &Path) -> Vec<TestResult> {
    ...
    // external set
    let set = ws
        .run_cli(bin, &["channel", "external", "set", "enabled", "true"])
        .await;
    results.push(pass(
        &format!("{}/set", suite),
        &format!("exit={}", set.exit_code),
    ...
    // external get
    let get = ws
        .run_cli(bin, &["channel", "external", "get", "enabled"])
        .await;
    results.push(pass(
        &format!("{}/get", suite),
        &format!("exit={}", get.exit_code),
