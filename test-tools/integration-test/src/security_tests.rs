...
pub async fn test_security_injection_sql() -> Vec<TestResult> {
    ...
    .await
    {
        Ok(content) => {
            ...
            let blocked = lower.contains("inject")
            ...
            results.push(pass(
                ...
                if blocked {
                    "SQL injection blocked".to_string()
                } else {
                    format!("Response ({} bytes)", content.len())
                },
            ));
        }
        Err(e) => {
...
pub async fn test_security_credential_leak() -> Vec<TestResult> {
    ...
    .await
    {
        Ok(content) => {
            ...
            if has_raw_key {
                results.push(fail(
                    &format!("{}/redacted", suite),
                    "API key not redacted in response",
                ));
            } else {
                results.push(pass(
                    &format!("{}/redacted", suite),
...
pub async fn test_security_ssrf_prevention() -> Vec<TestResult> {
    ...
    .await
    {
        Ok(content) => {
            let lower = content.to_lowercase();
            let blocked = lower.contains("ssrf")
            ...
            results.push(pass(
                &format!("{}/handled", suite),
                if blocked {
                    "SSRF blocked".to_string()
                } else {
                    format!("Handled ({} bytes)", content.len())
                },
            ));
        }
        Err(e) => {
