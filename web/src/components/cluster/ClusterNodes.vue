...
const addForm = ref({ address: '', name: '', role: 'worker', category: '' })
...
async function submitAddNode() {
  ...
  try {
    ...
    addForm.value = { address: '', name: '', role: 'worker', category: '' }
    await loadNodes()
  } catch (e: any) {
  ...
  <div v-if="!loading">
    ...
    <div v-for="node in filtered" :key="node.id">
      ...
      <div v-if="expandedId === node.id" style="margin:var(--space-2) 0">
        <NodeCard :node="node" @ping="pingNode" @remove="removeNode" />
      </div>
    </div>
    ...
    <div v-if="showAddModal" class="modal-overlay" @click.self="showAddModal = false">
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label>名称</label>
            <input class="form-input" v-model="addForm.name" placeholder="可选" />
          </div>
          <div class="form-group">
            <label>角色</label>
