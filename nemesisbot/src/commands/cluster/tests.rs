...
"#;
    ...
    fn test_format_token_long() {
        assert_eq!(crate::common::format_token("abcdefghijklmnop"), "abcd...mnop");
    }
    ...
    fn test_cluster_config_path_format() {
        ...
        assert!(path_str.contains("workspace") && path_str.contains("config") && path_str.contains("config.cluster.json"),
            "Expected workspace/config/config.cluster.json in path, got: {}", path_str);
    }
    ...
    fn test_cluster_dir_format() {
        ...
        assert!(dir_str.contains("workspace") && dir_str.contains("cluster"),
            "Expected workspace/cluster in path, got: {}", dir_str);
    }
    ...
    fn test_token_verify_match() {
        ...
        assert!(crate::common::constant_time_eq(stored.as_bytes(), saved_token.as_bytes()));
    }
    ...
    fn test_token_verify_mismatch() {
        ...
        let cfg: serde_json::Value = serde_json::from_str(&data).unwrap();
        let stored = cfg.get("token").and_then(|v| v.as_str()).unwrap();
        assert!(!crate::common::constant_time_eq(stored.as_bytes(), b"wrong-token"));
    }
    ...
    fn test_token_revoke_removes_from_config() {
        let tmp = TempDir::new().unwrap();
        let home = make_home(&tmp);
        write_cluster_config(&home, &serde_json::json!({"enabled": true, "token": "abc123"}));
    ...
    fn test_enable_checks_already_enabled() {
        ...
        let already = cfg.get("enabled").and_then(|v| v.as_bool()).unwrap_or(false);
        assert!(already);
    }
