...
mod wave_a {
    ...
    async fn project_path_and_is_running_and_loop_queries() {
        ...
        .unwrap();
        // 期望值走 canonicalize_for_compare：注册链 canonicalize 存卷上
        // 真实表示，而 tempfile 路径在 8.3 短名启用的卷上是短名（LOCALA~1）
        // ——文字比较会机器相关假红（远端用户名短无 8.3 缩短所以不显）。
        assert_eq!(
            mgr.project_path(&q.id)
                .map(|p| nemesis_path::paths::canonicalize_for_compare(&p)),
            Some(nemesis_path::paths::canonicalize_for_compare(&dir_q)),
        );
...
mod w5r2 {
    ...
    async fn w5_bridge_trait_create_spawn_reload_remove_roundtrip() {
        ...
        // 同 wave_a：canonicalize_for_compare 消 8.3 短名 vs 长名文字差。
        assert_eq!(
            bridge
                .project_path(&info.id)
                .map(|p| nemesis_path::paths::canonicalize_for_compare(&p)),
            Some(nemesis_path::paths::canonicalize_for_compare(&dir)),
        );
