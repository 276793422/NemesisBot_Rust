...
fn alternating_loop_different_errors_dont_accumulate() {
    let mut g = TurnGuard::new();
    assert!(
        g.record_tool_outcome("exec", Some("Error: error[E0432]: a"))
            .is_none()
    );
    assert!(
        g.record_tool_outcome("exec", Some("Error: error[E0425]: b"))
            .is_none()
    );
...
fn non_empty_final_answer_accepted() {
    let mut g = TurnGuard::new();
    assert!(matches!(
        g.check_final_answer("hello"),
        FinalAnswerVerdict::Accept
    ));
    assert!(matches!(
        g.check_final_answer("  x  "),
        FinalAnswerVerdict::Accept
    ));
    // Accept does not consume a retry budget.
    assert!(matches!(
        g.check_final_answer("again"),
        FinalAnswerVerdict::Accept
    ));
}
...
fn repeat_success_canonicalizes_whitespace() {
    let mut g = TurnGuard::new();
    assert!(
        g.record_write_success("write_file", "{ \"a\": 1 }")
            .is_none()
    );
...
fn text_repetition_nudges_then_resets() {
    ...
    assert!(
        g.check_text_repetition("完全不同的一句新内容，开始新任务。")
            .is_none()
    );
}
