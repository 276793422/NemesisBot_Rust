...
fn test_deepseek_thinking_disabled_workaround_characterization() {
    ...
    let body =
        provider.build_request_body(&[], &tools, "deepseek-v4-flash", &ChatOptions::default());
    assert!(
...
fn test_normalize_model() {
    ...
    assert_eq!(
        HttpProvider::normalize_model("claude3"),
        "claude-3-sonnet-20240229"
    );
    assert_eq!(
        HttpProvider::normalize_model("anthropic/claude3-opus"),
        "claude-3-opus-20240229"
    );
    assert_eq!(
        HttpProvider::normalize_model("my-custom-model"),
        "my-custom-model"
    );
...
fn test_normalize_model_aliases() {
    ...
    assert_eq!(
        HttpProvider::normalize_model("gpt35-turbo"),
        "gpt-3.5-turbo"
    );
    assert_eq!(
        HttpProvider::normalize_model("claude3"),
    ...
    assert_eq!(
        HttpProvider::normalize_model("claude3-opus"),
        "claude-3-opus-20240229"
    );
    assert_eq!(
        HttpProvider::normalize_model("claude3-sonnet"),
        "claude-3-sonnet-20240229"
    );
    assert_eq!(
        HttpProvider::normalize_model("claude3-haiku"),
...
fn test_normalize_model_preserves_unknown() {
    ...
    assert_eq!(
        HttpProvider::normalize_model("deepseek-chat"),
        "deepseek-chat"
    );
}
...
fn test_normalize_model_strips_prefix() {
    assert_eq!(HttpProvider::normalize_model("openai/gpt-4"), "gpt-4");
    assert_eq!(
        HttpProvider::normalize_model("anthropic/claude-3"),
        "claude-3"
    );
    assert_eq!(
        HttpProvider::normalize_model("deepseek/deepseek-chat"),
        "deepseek-chat"
    );
}
...
fn test_build_request_body_kimi_temperature() {
    ...
    let body = provider.build_request_body(&[], &[], "moonshot-v1", &ChatOptions::default());
...
async fn test_chat_connection_error() {
    ...
    let result = provider
        .chat(&messages, &[], "gpt-4", &ChatOptions::default())
        .await;
...
async fn test_chat_with_tools() {
    ...
    let result = provider
        .chat(&messages, &tools, "gpt-4", &ChatOptions::default())
        .await;
