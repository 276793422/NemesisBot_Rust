...
fn test_database_creates_parent_directory() {
    ...
    let _store =
        DataStore::open(&db_path).expect("Failed to create database with nested directories");
    assert!(db_path.exists());
...
fn test_query_summary_with_time_range() {
    ...
    // Query full range
    let summary = store
        .query_summary(1700000000, 1700000000 + 6 * 3600)
        .expect("Failed to query summary");
    assert_eq!(summary.total_requests, 5);

    // Query partial range
    let partial_summary = store
        .query_summary(1700000000, 1700000000 + 3 * 3600)
        .expect("Failed to query summary");
...
fn test_query_summary_with_error_responses() {
    ...
    store
        .insert_request_log(&success_log)
        .expect("Failed to insert success log");
...
fn test_query_trends_by_hour() {
    ...
    let trends = store
        .query_trends(1700000000, 1700000000 + 6 * 3600, "hour")
...
fn test_query_trends_by_day() {
    ...
    let trends = store
        .query_trends(1700000000, 1700000000 + 4 * 86400, "day")
...
fn test_query_trends_with_invalid_group_by() {
    ...
    let trends = store
        .query_trends(0, 2000000000, "invalid")
...
fn test_insert_log_with_streaming_flag() {
    ...
    store
        .insert_request_log(&streaming_log)
        .expect("Failed to insert streaming log");
    ...
    store
        .insert_request_log(&non_streaming_log)
...
fn test_query_summary_with_no_matching_data() {
    ...
    let summary = store
        .query_summary(1800000000, 1900000000)
...
fn test_query_summary_aggregation_accuracy() {
    ...
    store
        .insert_request_log(&log1)
        .expect("Failed to insert log1");
    store
        .insert_request_log(&log2)
        .expect("Failed to insert log2");
    store
        .insert_request_log(&log3)
        .expect("Failed to insert log3");
...
fn test_multiple_models_in_trends() {
    ...
    for i in 0..4 {
        ...
        store
            .insert_request_log(&log)
            .expect("Failed to insert gpt-4 log");
    }
    ...
    for i in 0..3 {
        ...
        store
            .insert_request_log(&log)
            .expect("Failed to insert gpt-3.5 log");
    }

    let trends = store
        .query_trends(1700000000, 1700000000 + 5 * 3600, "hour")
...
fn test_trend_data_format() {
    ...
    // Query hourly trends
    let trends = store
        .query_trends(log.created_at - 3600, log.created_at + 3600, "hour")
        .expect("Failed to query hourly trends");
