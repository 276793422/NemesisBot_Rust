...
//
// Safety contract (applies to EVERY function generated by sherpa_fn! below —
// documented here once instead of per-generated-fn; missing_safety_doc is
// suppressed inside the macro for the same reason): each pointer argument must
// be a valid, live sherpa-onnx object / NUL-terminated C string / sample
// buffer of the shape the corresponding sherpa-onnx C API expects, and must
// outlive the call. Returned pointers are owned by sherpa-onnx and must be
// released through the matching Destroy*/Free* wrapper.
// =============================================================================
...
macro_rules! sherpa_fn {
    ...
    ($(#[$meta:meta])* $name:ident($($arg:ident : $arg_ty:ty),*) -> $ret:ty) => {
        ...
        #[allow(clippy::missing_safety_doc)] // Safety 契约统一见上方宏注释
        $(#[$meta])*
...
/// # Safety
///
/// `config` must be a valid, fully initialized `SherpaOnnxOfflineTtsConfig`
/// (built by this module from owned `CString` fields) and must outlive the
/// call; the FFI target reads through it. The returned pointer is owned by
/// sherpa-onnx and must be freed with `SherpaOnnxDestroyOfflineTts`.
pub unsafe fn safe_create_offline_tts(
    config: *const SherpaOnnxOfflineTtsConfig,
...
/// # Safety
///
/// `tts` must be a live engine pointer returned by [`safe_create_offline_tts`]
/// and `text` a valid NUL-terminated C string; both are read by the FFI
/// target and must outlive the call. The returned pointer is owned by
/// sherpa-onnx and must be freed with
/// `SherpaOnnxDestroyOfflineTtsGeneratedAudio`.
pub unsafe fn safe_tts_generate_audio(
    tts: *const SherpaOnnxOfflineTts,
    text: *const libc::c_char,
    sid: libc::c_int,
    speed: f32,
