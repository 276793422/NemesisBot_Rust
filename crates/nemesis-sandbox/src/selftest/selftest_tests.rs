...
fn probes_unsandboxed_report_allowed_not_blocked() {
    let workspace = std::env::temp_dir().join(format!("nemesis_selftest_ws_{}", std::process::id()));
    ...
    assert!(!checks[2].blocked, "control probe must pass: {:?}", checks[2]);
    // 无沙盒：外部写探测如实报告「写入成功（未拦截）」。
    assert!(!checks[0].blocked, "unsandboxed outside-write must be allowed: {:?}", checks[0]);
