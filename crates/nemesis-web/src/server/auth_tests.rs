...
fn test_extract_request_token_header_first() {
    let req = req_with(
        ...
        &[("x-auth-token", "from-header"), ("authorization", "Bearer from-bearer")],
    );
    assert_eq!(
...
fn test_extract_request_token_empty_query_skipped() {
    ...
    let req = req_with("/api/status?token=", &[("authorization", "Bearer from-bearer")]);
...
async fn test_auth_wrong_token_rejected_401() {
    for (label, resp) in [
        ("查询参数错 token", router_oneshot("/api/status?token=wrong", None, None).await),
        ("头错 token", router_oneshot("/api/status", Some("wrong"), None).await),
        ("Bearer 错 token", router_oneshot("/api/status", None, Some("wrong")).await),
    ] {
...
async fn test_auth_valid_token_three_carriers_pass() {
    for (label, resp) in [
        ("头 token", router_oneshot("/api/status", Some("sekret"), None).await),
        ("查询参数 token", router_oneshot("/api/status?token=sekret", None, None).await),
        ("Bearer token", router_oneshot("/api/status", None, Some("sekret")).await),
    ] {
