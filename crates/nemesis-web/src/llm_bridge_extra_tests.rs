...
struct MockProvider {
    ...
    /// H4：adapter 透传到 provider 的 reasoning_effort（None 表示透传了
    /// None 或根本没带 options —— 用 absent/Some 区分需要看测试意图）。
    last_effort: Mutex<Option<String>>,
}
...
// ---------------------------------------------------------------------------
// H4 reasoning_effort 透传（M8 补测：L2 只钉了 CLI/WSAPI 侧，这里钉
// adapter 最后一跳 —— agent ChatOptions → provider ChatOptions 的字段搬运）
// ---------------------------------------------------------------------------

#[tokio::test]
async fn adapter_chat_passes_reasoning_effort_h4() {
    let mock = Arc::new(MockProvider::new("m", "p"));
    mock.set_response(text_response("ok"));
    let adapter = ProviderAdapter::new(mock.clone(), "m".to_string());

    // 带 options 且 effort=low → provider 收到 Some("low")。
    ...
    let _ = adapter
        ...
        .unwrap();
    assert_eq!(
        mock.last_effort.lock().unwrap().as_deref(),
        Some("low"),
        "H4: adapter 必须把 agent 侧 reasoning_effort 透传给 provider"
    );

    // 带 options 但 effort=None → provider 收到 None（不能残留上一次的值）。
    let opts = ChatOptions {
        reasoning_effort: None,
        ..Default::default()
    };
    let _ = adapter
        ...
        .unwrap();
    assert_eq!(
        mock.last_effort.lock().unwrap().as_ref(),
        None,
