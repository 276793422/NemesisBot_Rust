...
fn test_register_all_registers_16_handlers() {
    ...
    let expected = [
        "system", "config", "models", "channels", "identity", "tools", "scanner", "memory",
...
async fn test_config_set_field() {
    ...
    let result = handler
        .handle_cmd("set_field", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_config_save_and_get_roundtrip() {
    ...
    handler
        .handle_cmd("save", Some(cfg_data), &ctx)
        .await
        .unwrap();
...
async fn test_config_cors_commands() {
    ...
    handler
        .handle_cmd("cors.add", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    ...
    let data = serde_json::json!({ "origin": "https://example.com" });
    handler
        .handle_cmd("cors.remove", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    ...
    handler
        .handle_cmd("cors.toggle", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
}
...
async fn test_models_add_list_delete() {
    ...
    handler
        .handle_cmd("add", Some(holder), &ctx)
        .await
        .unwrap()
        .unwrap();
    handler
        .handle_cmd(
            "set_default",
            Some(serde_json::json!({ "name": "default-holder" })),
            &ctx,
        )
        .await
        .unwrap()
    ...
    let result = handler
        .handle_cmd("delete", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_models_add_duplicate() {
    ...
    handler
        .handle_cmd("add", Some(data.clone()), &ctx)
        .await
        .unwrap();
...
async fn test_mcp_add_list_delete_server() {
    ...
    let result = handler
        .handle_cmd("server.add", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert!(result["added"].as_bool().unwrap());
    ...
    let result = handler
        .handle_cmd("server.update", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert!(result["updated"].as_bool().unwrap());
    ...
    let result = handler
        .handle_cmd("server.delete", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert!(result["deleted"].as_bool().unwrap());
...
async fn test_mcp_add_duplicate() {
    ...
    handler
        .handle_cmd("server.add", Some(data.clone()), &ctx)
        .await
        .unwrap();
...
async fn test_skills_detail() {
    ...
    let result = handler
        .handle_cmd("detail", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_skills_uninstall() {
    ...
    let result = handler
        .handle_cmd("uninstall", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_memory_status_empty() {
    ...
    let result = handler
        ...
        .unwrap();
    assert!(
        !result["document_memory"]["directory_exists"]
            .as_bool()
            .unwrap()
    );
...
async fn test_memory_documents_and_get_save() {
    ...
    let result = handler
        .handle_cmd("document.get", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    ...
    let result = handler
        .handle_cmd("document.save", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert!(result["saved"].as_bool().unwrap());
...
async fn test_tasks_boot_get_save() {
    ...
    let result = handler
        .handle_cmd("boot.save", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert!(result["saved"].as_bool().unwrap());
...
async fn test_tasks_heartbeat_get_save() {
    ...
    handler
        .handle_cmd("heartbeat.save", Some(data), &ctx)
        .await
        .unwrap();

    let result = handler
        .handle_cmd("heartbeat.get", None, &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_tasks_cron_add_list_update_delete() {
    ...
    let result = handler
        .handle_cmd("cron.add", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert!(result["added"].as_bool().unwrap());
    ...
    let result = handler
        .handle_cmd("cron.update", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert!(result["updated"].as_bool().unwrap());
    ...
    let result = handler
        .handle_cmd("cron.delete", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert!(result["deleted"].as_bool().unwrap());
...
async fn test_logs_requests_pagination() {
    ...
    let result = handler
        .handle_cmd("requests", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_logs_request_detail() {
    ...
    let result = handler
        .handle_cmd("request_detail", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
...
impl IntegrationRouter {
    ...
    async fn dispatch(
        ...
        self.router
            .dispatch(&msg, &self.ctx, &self.send_queue)
            .await;
...
async fn integration_dispatch_unknown_module() {
    ...
    let resp = router
        .dispatch("nonexistent_module", "cmd", "r1", None)
        .await;
...
async fn integration_dispatch_identity_lifecycle() {
    ...
    let data = router.dispatch_ok("identity", "list", None).await;
    let identity_doc = data["documents"]
        .as_array()
        .unwrap()
        .iter()
        .find(|d| d["name"] == "IDENTITY.md")
        .unwrap();
...
async fn integration_dispatch_models_lifecycle() {
    ...
    let entry = models
        .iter()
        .find(|m| m["model_name"] == "test-gpt")
        .expect("test-gpt present");
...
async fn concurrency_models_rapid_add_delete() {
    ...
    handler
        .handle_cmd(
            "set_default",
            Some(serde_json::json!({ "name": "base-holder" })),
            &ctx,
        )
        .await
        .unwrap();
    ...
    for i in 0..20 {
        ...
        handler
            .handle_cmd("add", Some(add_data), &ctx)
            .await
            .unwrap();

        let del_data = serde_json::json!({ "name": &name });
        handler
            .handle_cmd("delete", Some(del_data), &ctx)
            .await
            .unwrap();
    }
...
async fn concurrency_cron_rapid_add_delete() {
    ...
    for i in 0..10 {
        ...
        let result = handler
            .handle_cmd("cron.add", Some(add_data), &ctx)
            .await
            .unwrap()
            .unwrap();
        let job_id = result["job"]["id"].as_str().unwrap().to_string();
        ...
        handler
            .handle_cmd("cron.delete", Some(del_data), &ctx)
            .await
            .unwrap();
    }
...
async fn concurrency_mcp_rapid_add_delete() {
    ...
    for i in 0..10 {
        ...
        handler
            .handle_cmd("server.add", Some(add_data), &ctx)
            .await
            .unwrap();

        let del_data = serde_json::json!({ "name": format!("s-{}", i) });
        handler
            .handle_cmd("server.delete", Some(del_data), &ctx)
            .await
            .unwrap();
    }
...
async fn test_models_set_default() {
    ...
    let result = handler
        .handle_cmd("set_default", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_channels_update() {
    ...
    let result = handler
        .handle_cmd("update", Some(update_data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert!(result["updated"].as_bool().unwrap());
...
async fn test_memory_vector_status_no_config() {
    ...
    let ctx = make_ctx(&dir);
    let result = handler
        .handle_cmd("vector.status", None, &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_memory_vector_search_stub() {
    ...
    let result = handler
        .handle_cmd("vector.search", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_security_audit_with_filter() {
    ...
    let result = handler
        .handle_cmd("audit", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn hicon_50_parallel_logs_reads() {
    ...
    for _ in 0..50 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            h.handle_cmd("requests", Some(serde_json::json!({ "limit": 10 })), &ctx)
                .await
        }));
    }
...
async fn hicon_mixed_50_concurrent_read_write() {
    ...
    assert!(
        errors < 25,
        "too many errors during concurrent access: {}",
        errors
    );
}
...
async fn hicon_30_parallel_mixed_handlers() {
    ...
    for _ in 0..5 {
        let ctx = ctx.clone();
        handles.push(tokio::spawn(async move {
            let r = config::ConfigHandler::new()
                .handle_cmd("get", None, &ctx)
                .await;
            ("config", r)
        }));
    ...
    for _ in 0..5 {
        let ctx = ctx.clone();
        handles.push(tokio::spawn(async move {
            let r = identity::IdentityHandler
                .handle_cmd("list", None, &ctx)
                .await;
            ("identity", r)
        }));
...
async fn test_forge_status_with_artifacts() {
    ...
    assert!(
        artifacts
            .iter()
            .any(|a| a["name"] == "skills" && a["type"] == "directory")
    );
    assert!(
        artifacts
            .iter()
            .any(|a| a["name"] == "config.json" && a["type"] == "file")
    );
}
...
async fn test_tasks_cron_update_multiple_fields() {
    ...
    handler
        .handle_cmd("cron.update", Some(update_data), &ctx)
        .await
        .unwrap();
...
async fn test_tasks_cron_list_corrupted() {
    ...
    assert!(
        result.is_ok(),
        "corrupted store should degrade gracefully: {:?}",
        result
    );
...
async fn test_tasks_independent_files() {
    ...
    handler
        .handle_cmd(
            "boot.save",
            Some(serde_json::json!({ "content": "# Boot" })),
            &ctx,
        )
        .await
        .unwrap();
    handler
        .handle_cmd(
            "heartbeat.save",
            Some(serde_json::json!({ "content": "# Heart" })),
            &ctx,
        )
        .await
        .unwrap();
    ...
    handler
        .handle_cmd(
            "boot.save",
            Some(serde_json::json!({ "content": "# Boot v2" })),
            &ctx,
        )
        .await
        .unwrap();
...
async fn test_security_audit_risk_level_filter() {
    ...
    assert_eq!(
        result["total"], 4,
        "should return all 4 entries without risk_level filter"
    );
}
...
async fn test_logs_security_with_filter() {
    ...
    let result = handler
        .handle_cmd("security", Some(data), &ctx)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(result["total"], 1);
...
async fn stress_100_write_integrity() {
    ...
    let names: std::collections::HashSet<_> = models
        .iter()
...
async fn stress_50_concurrent_write_with_verification() {
    ...
    assert!(
        parsed.is_ok(),
        "config.json should be valid JSON after concurrent writes"
    );
...
async fn test_logs_chain_list_three_valid_events() {
    ...
    let handler = LogsHandler;
    let res = handler
        .handle_cmd("chain_list", Some(serde_json::json!({})), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_logs_session_detail_empty_when_no_memory_manager() {
    ...
    let res = handler
        .handle_cmd(
            "session_detail",
            Some(serde_json::json!({ "session": "web:abc" })),
            &ctx,
        )
        .await
        .unwrap()
...
async fn test_logs_cluster_task_list_empty_when_no_dir() {
    ...
    let handler = LogsHandler;
    let res = handler
        .handle_cmd("cluster_task_list", None, &ctx)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(res["entries"].as_array().unwrap().len(), 0);
...
fn test_parse_cluster_dir_name_with_ms() {
    let (ts, task) =
        crate::handlers::logs::parse_cluster_dir_name("2026-06-17_14-23-45-123_taskABC").unwrap();
