...
mod tests {
    ...
    #[test]
    fn test_strip_emoji() {
        assert_eq!(strip_emoji("🧠 Your Identity & Memory"), "your identity & memory");
    ...
    fn test_parse_sections() {
        ...
        let parsed = parse_sections(content);
        assert!(parsed.preamble.contains("You are a test agent"), "preamble should capture role description");
    ...
    fn test_convert_real_agent_file() {
        ...
        // Parse sections
        let parsed = parse_sections(&content);
        assert!(parsed.sections.len() >= 4, "expected at least 4 sections, got {}", parsed.sections.len());

        // Verify section titles contain expected keywords
        let titles: Vec<String> = parsed.sections.iter().map(|s| strip_emoji(&s.title)).collect();
        assert!(titles.iter().any(|t| t.contains("identity")), "missing identity section");
        assert!(
            titles.iter().any(|t| t.contains("critical rule") || t.contains("communication")),
    ...
    fn test_convert_real_agent_files_batch() {
        ...
        let test_cases = [
            ("C:/AI/NemesisBot/agency-agents/engineering/engineering-code-reviewer.md", "Code Reviewer"),
            ("C:/AI/NemesisBot/agency-agents/security/security-architect.md", "Security Architect"),
            ("C:/AI/NemesisBot/agency-agents/marketing/marketing-douyin-strategist.md", "Douyin Strategist"),
        ];
        ...
        for (path, expected_name_part) in &test_cases {
            ...
            assert!(full_agent.contains("首次运行"), "AGENT.md missing system defaults");
        }
    }
    ...
    fn test_local_management() {
        ...
        std::fs::write(std::path::Path::new(&ws).join("IDENTITY.md"), "# Test Identity").unwrap();
        ...
        // Test file.get
        let result = handler.cmd_file_get(&ws, "default", "IDENTITY.md").unwrap().unwrap();
