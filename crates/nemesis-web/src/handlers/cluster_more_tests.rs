...
async fn test_firewall_check_invalid_config_falls_back_to_defaults() {
    ...
    let dir = tempfile::tempdir().unwrap();
    ensure_config_dir(dir.path());
    std::fs::write(dir.path().join("config/config.cluster.json"), "not json").unwrap();
    let ctx = make_ctx(&dir);
...
async fn test_events_recent_limit_zero() {
    ...
    let result = handler
        .handle_cmd("events.recent", Some(serde_json::json!({"limit": 0})), &ctx)
        .await
        .unwrap()
        .unwrap();
...
async fn test_nodes_detail_unknown_node_errors() {
    ...
    let err = handler
        .handle_cmd(
            "nodes.detail",
            Some(serde_json::json!({"node_id": "missing"})),
            &ctx,
        )
        .await
        .unwrap_err();
...
async fn test_nodes_detail_known_node_returns_info() {
    ...
    let result = handler
        .handle_cmd(
            "nodes.detail",
            Some(serde_json::json!({"node_id": "n7"})),
            &ctx,
        )
        .await
        .unwrap()
...
async fn test_nodes_remove_known_returns_true() {
    ...
    let result = handler
        .handle_cmd(
            "nodes.remove",
            Some(serde_json::json!({"node_id": "n8"})),
            &ctx,
        )
        .await
        .unwrap()
...
async fn test_tasks_list_status_filter_no_match() {
    ...
    let result = handler
        .handle_cmd(
            "tasks.list",
            Some(serde_json::json!({"status_filter":"completed"})),
            &ctx,
        )
        .await
        .unwrap()
...
async fn test_tasks_detail_unknown_task_errors() {
    ...
    let err = handler
        .handle_cmd(
            "tasks.detail",
            Some(serde_json::json!({"task_id":"nope"})),
            &ctx,
        )
        .await
        .unwrap_err();
...
async fn test_node_update_identity_role_worker() {
    ...
    let result = handler
        .handle_cmd(
            "node.update_identity",
            Some(serde_json::json!({"role":"worker"})),
            &ctx,
        )
        .await
        .unwrap()
...
async fn test_node_update_identity_invalid_role_errors() {
    ...
    let err = handler
        .handle_cmd(
            "node.update_identity",
            Some(serde_json::json!({"role":"supervisor"})),
            &ctx,
        )
        .await
        .unwrap_err();
...
async fn test_node_update_identity_category() {
    ...
    let result = handler
        .handle_cmd(
            "node.update_identity",
            Some(serde_json::json!({"category":"production"})),
            &ctx,
        )
        .await
        .unwrap()
...
async fn test_node_update_identity_tags_filters_empty() {
    ...
    let result = handler
        .handle_cmd(
            "node.update_identity",
            Some(serde_json::json!({"tags":["prod","edge",""]})),
            &ctx,
        )
        .await
        .unwrap()
