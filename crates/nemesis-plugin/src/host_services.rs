...
pub const HOST_SERVICES_VERSION: u32 = 2;
...
pub fn build_host_services(workspace_dir: &Path) -> HostServices {
    ...
    HostServices {
        ...
        decode_png: None, // caller must set if available
    }
}
...
mod tests {
    ...
    fn get_plugin_data_dir_null_inputs_return_negative() {
        let hs = build_host_services(&std::env::temp_dir());
        let n = (hs.get_plugin_data_dir.unwrap())(
            std::ptr::null(),
            std::ptr::null_mut(),
            0,
        );
    ...
    fn tray_callbacks_copy_and_clone() {
        ...
        let tc = TrayCallbacks { user_data: std::ptr::null_mut(), on_menu_click: cb };
        let tc2 = tc; // Copy
