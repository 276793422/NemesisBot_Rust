...
"#;
    ...
    assert_eq!(
        ...
        "Clean content should score 100 for security"
    );
    // Should have reasonable completeness
    assert!(
        result.completeness.score > 0.0,
...
fn test_score_empty_content() {
    ...
    assert_eq!(
        result.clarity.score, 0.0,
        "Empty content should score 0 for clarity"
    );
}
...
fn test_score_with_metadata() {
    ...
    assert!(
        result.completeness.score >= 30.0,
        "Metadata should boost completeness"
    );
}
...
fn test_has_heading_pattern() {
    ...
    assert!(has_heading_pattern(
        content,
        r"(?i)^#+\s*(?:overview|summary)"
    ));
    assert!(!has_heading_pattern(
        content,
        r"(?i)^#+\s*(?:inputs?|outputs?)"
    ));
}
...
fn test_completeness_score_details() {
    ...
    assert_eq!(
        score.score, 90.0,
        "Should find 6 indicators at 15 each = 90"
    );
}
...
fn test_testing_score_with_test_mentions() {
    ...
    assert!(
        score.score >= 60.0,
        "Should have good testing score, got {}",
        score.score
    );
}
...
fn test_testing_validation_rules() {
    let score =
        QualityScorer::score_testing("All input must be validated. Required fields constraint.");
    assert!(score.score >= 20.0);
}

#[test]
fn test_testing_edge_cases() {
    let score = QualityScorer::score_testing(
        "Handle edge case for empty input. Check boundary conditions.",
    );
    assert!(score.score >= 20.0);
}
...
fn test_clarity_with_single_code_block() {
    ...
    assert!(
        score.score >= 10.0,
        "Single code block should give at least 10"
    );
}
...
fn test_is_consistent_script_with_other_chars() {
    ...
    let mixed =
        "\u{0410}\u{0411}\u{0412}\u{0413}\u{0414}\u{0415}\u{0416}\u{0417}\u{0418}\u{0419} Hello";
